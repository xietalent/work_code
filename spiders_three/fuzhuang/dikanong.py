...
class Dikanong(object):
    ...
    def process_request(self):
        ...
        self.browser.get("https://www.decathlon.com.cn/zh/login")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.ID,"login")))
        # 选择登录方式
        ...
        if loginway == "1":
            try:
                self.browser.find_element_by_id("login").send_keys(usename)
                sleep(0.1)
                self.browser.find_element_by_xpath("//div[@id='validate-block']/button[1]").click()
                sleep(0.5)
                self.browser.find_element_by_id("password").send_keys(passwd)
                sleep(0.5)
                self.browser.find_element_by_xpath("//div[@id='signin-block']/button[1]").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\dikanong_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\dikanong_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        with open(r"E:\code\spiders_three\text\dikanong_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()

        # with open(r"E:\code\spiders_three\text\dikanong_info.txt", 'r', encoding="utf-8") as f:
        #     html = f.read()
