...
class Botao(object):
    ...
    def process_request(self):
        ...
        self.browser.get("http://www.podinns.com/Account/LogOn?")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.ID,"divLogin")))
        # 选择登录方式
        ...
        if loginway == "1":
            try:
                ...
                self.browser.find_element_by_id("ImgeCode").send_keys(img_code)
                sleep(0.1)
                self.browser.find_element_by_xpath("//form[@id='form1']/div[@id='divLogin']/a").click()
    ...
    def req_IHG(self):
        ...
        url = "http://www.podinns.com/Member/MyVoucher"
        headers = {
            'cookie': cookie_str,
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36',
        }
        tuniu_html = requests.get(url=url, headers=headers)
        with open(r"E:\code\spiders_three\text\rujia_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\rujia_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
    ...
    def account_info(self):
        self.browser.get("http://www.podinns.com/Member/MyFenHistory")
        with open(r'E:\code\spiders_three\cookie_bag\rujia_cook.txt', 'r', encoding='utf-8') as fp:
            list_cookies = json.loads(fp.read())
            for c in list_cookies:
                self.browser.add_cookie(c)
        #历史订单
        self.browser.get("http://www.podinns.com/Member/MyOrderHistory")
        sleep(3)
        #积分兑换记录
        self.browser.get("http://www.podinns.com/Member/MyFenShopHistory")
        sleep(3)
        #我的优惠券
        self.browser.get("http://www.podinns.com/Member/MyVoucher")
