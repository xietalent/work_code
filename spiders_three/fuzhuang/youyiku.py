...
class Anta(object):
    ...
    def process_request(self):
        ...
        self.browser.get("https://www.uniqlo.cn/login.html?")
        usename, passwd = self.user_info()
        # WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.CLASS_NAME,"user_name")))
        # 选择登录方式
        ...
        if loginway == "1":
            try:
                self.browser.find_element_by_xpath("//div[@class='form-wrap']//div[2]/div/div[1]/input").send_keys(usename)
                sleep(0.2)
                #滑动验证
                try:
                    ...
                    pass
                #获取验证码
                self.browser.find_element_by_xpath("//div[@class='form-wrap']//form[1]/div[4]/div/button[1]").click()
                sleep(0.1)
                ver_code = self.get_vercode()
                self.browser.find_element_by_xpath("//div[@class='form-wrap']//div[2]/input").send_keys(ver_code)
                self.browser.find_element_by_xpath("//div[@class='form-wrap']//div[6]/div/button[1]").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\youyiku_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\youyiku_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        url = "http://www.youyiku.cn/ucenter/mycredit.html"
        headers = {
            'cookie': cookie_str,
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36',
        }
        tuniu_html = requests.get(url=url, headers=headers)
        with open(r"E:\code\spiders_three\text\youyiku_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\youyiku_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
...
class Vercode(object):
    ...
    def start_move(self, distance):
        ...
        #点击小图片滑动
        element = self.browser.find_element_by_xpath("//div[@class='h-slider-bar']/div[1]")
