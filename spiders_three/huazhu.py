...
class Huazhu(object):
    ...
    def process_request(self):
        ...
        self.browser.get("https://passport.huazhu.com/?redirectUrl=http://www.huazhu.com")
        ...
        if loginway == "1":
            try:
                self.browser.find_element_by_name("username").send_keys(usename)
                sleep(0.1)
                self.browser.find_element_by_xpath("//section[@class='US_signin']/div[1]/div[1]/aside[1]/section[2]/div[1]/div[2]/div[1]/input[1]").send_keys(passwd)
                sleep(0.1)
                self.browser.find_element_by_id("s_submit").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\huazhu_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\huazhu_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        url = "https://passport.huazhu.com/?redirectUrl=http://www.huazhu.com"
        headers = {
            'cookie': cookie_str,
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36'
        }
        tuniu_html = requests.get(url=url, headers=headers)
        with open(r"E:\code\spiders_three\text\huazhu_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\huazhu_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
    ...
    def account_info(self):
        self.browser.get("https://my.huazhu.com/PersonalInfo")
        sleep(0.1)
        with open(r'E:\code\spiders_three\cookie_bag\huazhu_cook.txt', 'r', encoding='utf-8') as fp:
            list_cookies = json.loads(fp.read())
