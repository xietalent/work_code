...
class Mtime(object):
    ...
    def process_request(self):
        self.browser.get("https://passport.mtime.com/member/signin/?redirectUrl=http%3A%2F%2Fwww.mtime.com%2F")
        usename,passwd = self.user_info()
        # loginway = input("请选择登录方式(1,2):")
        loginway = "1"
        if loginway =="1":
            try:
                self.browser.find_element_by_id("loginEmailText").send_keys(usename)
                sleep(0.1)
                self.browser.find_element_by_id("loginPasswordText").send_keys(passwd)
                sleep(0.1)
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\Mtime_cook.txt', 'w') as fp:
    ...
    #
    #     url = "http://vip.mtime.com/"
    #     headers = {
    #         'cookie': cookie_str,
    #         'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36'
    #     }
    #     tuniu_html = requests.get(url=url, headers=headers)
    #     with open(r"E:\code\spiders_three\text\Mtime_info.txt",'w',encoding="utf-8")  as fp:
    #         fp.write(tuniu_html.text)
    #         fp.close()
    #     print(tuniu_html.text)
    #     # my_info = tuniu_html.text
    #
    #     with open(r"E:\code\spiders_three\text\Mtime_info.txt", 'r', encoding="utf-8") as f:
    #         html = f.read()
    #
    ...
    def account_info(self):
        # self.browser.find_element_by_xpath("//dl[@class='headbarnav']/dd[8]/a").click()
