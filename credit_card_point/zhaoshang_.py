...
class Zhanoshang_bank():
    ...
    def process_request(self):
        ...
        # WebDriverWait(self.browser, 5, 1).until(EC.element_to_be_clickable((By.ID, "banner1")))
        ...
        self.input_user()
        # WebDriverWait(self.browser, 5, 1).until(EC.element_to_be_clickable((By.ID, "btnSendCode")))
        sleep(0.1)
        # 输入手机验证码
        try:
    ...
    def score_history(self):
        ...
        self.browser.find_element_by_xpath(
            "//ul[@class='chosen-results']/li[@class='active-result'][{}]".format(month)).click()
        sleep(0.5)
    ...
    def transaction_record(self):
        ...
        try:
            self.browser.find_element_by_xpath(
                "//div[@class='page-panel-header']/sapn[@class='page-panel-header-more']/a[2]").click()
        except:
...
if __name__ == '__main__':
    ...
    try:
        ...
        ts = time.clock() - ts6
        ts = round(ts, 2)
        print("总耗时(去除最后一年记录查询时间)为:{}s".format(ts))
    except:
        pass
    del s
