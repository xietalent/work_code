...
class KFC():
    ...
    def process_request(self):
        ...
        self.browser.get("https://www.4008823823.com.cn/kfcios/customerCenter.action")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.CLASS_NAME,"login_btn")))
        # 普通登录
        sleep(0.5)
        try:
            self.browser.find_element_by_class_name("input_username").send_keys(usename)
            sleep(0.1)
            #获取短信验证码
            self.browser.find_element_by_class_name("getcode").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\kfc_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\kfc_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        url = "http://www.4008823823.com.cn/kfcios/customerFromAgree.action"
        headers = {
            'cookie': cookie_str,
        ...
        with open(r"E:\code\spiders_three\text\kfc_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\kfc_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
