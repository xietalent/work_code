...
class Botao(object):
    ...
    def process_request(self):
        ...
        self.browser.get("https://login.bthhotels.com/login/index?")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.ID,"btnsubmitA")))
        # 选择登录方式
        ...
        if loginway == "1":
            try:
                self.browser.find_element_by_id("AccountNo_PC").send_keys(usename)
                sleep(0.1)
                self.browser.find_element_by_id("Pwd_PC").send_keys(passwd)
                sleep(0.1)
                img_code = self.get_img()
                self.browser.find_element_by_id("VerifyCode").send_keys(img_code)
                sleep(0.1)
                self.browser.find_element_by_id("btnsubmitA").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\zy_guoji_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\zy_guoji_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        with open(r"E:\code\spiders_three\text\zy_guoji_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\zy_guoji_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
