...
class Givenchy():
    ...
    def process_request(self):
        self.browser.get("https://www.givenchy.com/apac/zh/loginpage?")
        ...
        #尝试登陆
        try:
            self.browser.find_element_by_xpath("//div[@class='login-box']/form[@id='dwfrm_login']//div[1]/div[1]/div[1]/input").send_keys(username)
            sleep(0.1)
            self.browser.find_element_by_xpath("//div[@class='login-box']/form[@id='dwfrm_login']//div[2]/div[1]/div[1]/input").send_keys(passwd)
            sleep(0.5)
            self.browser.find_element_by_class_name("//button[@class='form-button']").click()
            # sleep(5)
            #点击登录
