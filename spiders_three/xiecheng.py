...
class Aiqiyi(object):
    ...
    def process_request(self):
        ...
        self.browser.get("https://passport.ctrip.com/user/login?")
        sleep(1)
        username,passwd = self.user_info()
        try:
            WebDriverWait(self.browser, 8, 0.5).until(EC.element_to_be_clickable((By.ID, 'nsubmit')))
        except:
            return  self.process_request()

        login_way = "1"
        if login_way =="1":
            self.browser.find_element_by_xpath("//div[@id='normalview']/dl[1]/dd/input[@id='nloginname']").send_keys(username)
            sleep(0.2)
            self.browser.find_element_by_id("npwd").send_keys(passwd)
    ...
    #积分查询
    def my_score(self):
        # self.browser.find_element_by_class_name("set-text").click()
        sleep(0.5)
        score_html = self.browser.page_source
        score_tree = etree.HTML(score_html)
        #个人信息
        divs_a = score_tree.xpath("//div[@id='memberAccount']/ul[@class='account-box']")
        items_a = []
        for div in divs_a:
            item_a ={}
            #礼品卡
            username = div.xpath("./li[1]/a[@class='f-blue']/span/text()")[0].strip()
            #现金cash
            available_score = div.xpath("./li[2]/a[@class='f-blue']/span/text()")[0].strip()
            # #返现
            member_info = div.xpath("./li[3]/a[@class='f-blue']/span/text()")[0].strip()
            # #积分
            score_xianzhi = div.xpath("./li[4]/a[@class='f-blue']/span/text()")[0].strip()

            print("礼品卡:{}".format(username))
            print("现金额:{}".format(available_score))
            print("返现:{}".format(member_info))
            print("积分数为:{}".format(score_xianzhi))
    ...
    def start_spider(self):
        ...
        while True:
            ...
            # sleep()
            try:
                WebDriverWait(self.browser, 8, 0.5).until(EC.element_to_be_clickable((By.CLASS_NAME, 'base_nav')))
                print("登录成功")
                break
            except:
                pass
