...
class Aiqiyi(object):
    ...
    def process_request(self):
        ...
        self.browser.get("http://review.qunar.com/mall/index.htm")
        sleep(1)
        ...
        self.browser.find_element_by_xpath("//div[@class='user_info']/a[@class='login']").click()
        sleep(1)
        self.browser.find_element_by_xpath("//div[@class='helpbar']/a[@class='pwd-login']").click()
        sleep(1)
        # login_way = input("请选择登录方式(1:密码登录/2:短信验证码):")
        login_way = "1"
        if login_way =="1":
            self.browser.find_element_by_xpath("//input[@class='textbox textbox_focus']").send_keys(username)
            sleep(0.2)
            self.browser.find_element_by_xpath("//input[@name='password']").send_keys(passwd)
            sleep(0.2)
            imgs = self.get_img()
            sleep(0.2)
