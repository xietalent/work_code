...
class Lining(object):
    ...
    def process_request(self):
        ...
        self.browser.get("https://store.lining.com/shop/login.html?redirect_url=https://store.lining.com/")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.ID,"login_btn")))
        # 选择登录方式
        ...
        if loginway == "1":
            try:
                ...
                self.browser.find_element_by_id("yzm").send_keys(img_code)
                sleep(0.1)
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\lining_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\lining_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        url = "https://store.lining.com/shop/memberCenter.html?operFlg=6"
        headers = {
            'cookie': cookie_str,
        ...
        with open(r"E:\code\spiders_three\text\lining_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\lining_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
    ...
    def account_info(self):
        self.browser.get("https://store.lining.com/shop/memberCenter.html?operFlg=6")
        with open(r'E:\code\spiders_three\cookie_bag\lining_cook.txt', 'r', encoding='utf-8') as fp:
            list_cookies = json.loads(fp.read())
        ...
        sleep(3)
        #我的余额
        self.browser.get("https://store.lining.com/shop/memberCenter.html?operFlg=7")
        sleep(3)
        #我的订单
        self.browser.get("https://store.lining.com/shop/member_order_list.php?orderflg=1")
    ...
    def start(self):
        ...
        while True:
            ...
            try:
                WebDriverWait(self.browser,5,0.5).until(EC.element_to_be_clickable((By.ID,'slide-goods-amount')))
                # sleep(3)
                # print("登录结束")
                self.get_cookie()
                break
