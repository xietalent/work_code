...
class IHG(object):
    ...
    def process_request(self):
        self.browser.get("https://www.ihg.com/hotels/cn/zh/reservation")
        usename, passwd = self.user_info()
        self.browser.find_element_by_class_name("logIn-link").click()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.CLASS_NAME,"form-control")))
        # 选择登录方式
        # loginway = input("请选择登录方式(1,2):")
        loginway = "1"
        if loginway == "1":
            try:
                self.browser.find_element_by_id("UHF_username").send_keys(usename)
                sleep(0.1)
                self.browser.find_element_by_id("UHF_password").send_keys(passwd)
                sleep(0.1)
                self.browser.find_element_by_xpath("//div[1]/form[1]//div[@class='row'][4]//div/button").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\IHG_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        ...
        url = "https://www.ihg.com/rewardsclub/cn/zh/account-mgmt/home"
        headers = {
            'cookie': cookie_str,
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36'
        }
        tuniu_html = requests.get(url=url, headers=headers)
        with open(r"E:\code\spiders_three\text\IHG_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\IHG_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
