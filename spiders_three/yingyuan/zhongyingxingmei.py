...
class ZhongyingXingmei(object):
    ...
    def process_request(self):
        ...
        self.browser.get("http://www.ixingmei.com/member/index.php?act=login")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.CLASS_NAME,"submit")))
        # 选择登录方式
        ...
        if loginway == "1":
            try:
                self.browser.find_element_by_id("user_name").send_keys(usename)
                sleep(0.1)
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\xingmei_cook.txt', 'w') as fp:
    ...
    def req_IHG(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\xingmei_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        with open(r"E:\code\spiders_three\text\xingmei_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        with open(r"E:\code\spiders_three\text\xingmei_info.txt", 'r', encoding="utf-8") as f:
            html = f.read()
    ...
    def account_info(self):
        self.browser.get("http://www.ixingmei.com/member/index.php?act=member_points&op=index")
        with open(r'E:\code\spiders_three\cookie_bag\xingmei_cook.txt', 'r', encoding='utf-8') as fp:
            list_cookies = json.loads(fp.read())
        ...
        sleep(3)
        #消费记录
        self.browser.get("http://www.ixingmei.com/member/index.php?act=consume&op=index")
        sleep(3)
        #代金券页
        self.browser.get("http://www.ixingmei.com/member/index.php?act=member_voucher&op=index")
    ...
    def start(self):
        ...
        while True:
            ...
            try:
                # sleep(5)
                # WebDriverWait(self.browser,15,0.5).until(EC.element_to_be_clickable((By.CLASS_NAME,'site-menu')))
                # print("登录结束")
                # self.get_cookie()
                break
            except Exception as e:
