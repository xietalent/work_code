...
class Guomei():
    ...
    def process_request(self):
        ...
        self.browser.get("https://login.gome.com.cn/login?")
        usename, passwd = self.user_info()
        WebDriverWait(self.browser,5,1).until(EC.element_to_be_clickable((By.ID,"tabRight")))
        # sleep(5)
        # 普通登录
        sleep(0.5)
        try:
            self.browser.find_element_by_xpath("//div[@id='tabRight']/a").click()
            sleep(0.2)
            self.browser.find_element_by_id("loginName").send_keys(usename)
            sleep(0.1)
            #密码
            self.browser.find_element_by_id("loginPassword").send_keys(passwd)
            sleep(0.1)
            # #滑动验证码
            # try:
            ...
            self.browser.find_element_by_class_name("btnnuw").click()
    ...
    def get_cookie(self):
        ...
        print(type(cookies))
        with open(r'E:\code\spiders_three\cookie_bag\guomei_cook.txt', 'w') as fp:
    ...
    def req_guomei(self):
        # 使用cookie模拟登录状态
        with open(r'E:\code\spiders_three\cookie_bag\guomei_cook.txt', 'r', encoding='utf-8') as fp:
        ...
        url = "https://myhome.gome.com.cn/member/profileHome?intcmp=myhome-public01051"
        headers = {
            'cookie': cookie_str,
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36',
        }
        tuniu_html = requests.get(url=url, headers=headers)
        with open(r"E:\code\spiders_three\text\guomei_info.txt", 'w', encoding="utf-8")  as fp:
            fp.write(tuniu_html.text)
            fp.close()
        print(tuniu_html.text)
        # my_info = tuniu_html.text

        # with open(r"E:\code\spiders_three\text\guomei_info.txt", 'r', encoding="utf-8") as f:
        #     html = f.read()
    ...
    def account_info(self):
        self.browser.get("https://myhome.gome.com.cn/member/profileHome?intcmp=myhome-public01051")
        with open(r'E:\code\spiders_three\cookie_bag\guomei_cook.txt', 'r', encoding='utf-8') as fp:
            list_cookies = json.loads(fp.read())
            for c in list_cookies:
                self.browser.add_cookie(c)
        self.browser.get("https://myhome.gome.com.cn/member/profileHome?intcmp=myhome-public01051o")
        sleep(3)
        #我的优惠券
        self.browser.get("https://myhome.gome.com.cn/member/gomeMyCoupon")
        sleep(3)
        #我的美豆
        self.browser.get("https://myhome.gome.com.cn/member/pointConvert")
    ...
    def start(self):
        ...
        while True:
            ...
            try:
                WebDriverWait(self.browser,15,0.5).until(EC.element_to_be_clickable((By.ID,'gome-top-right')))
