...
class Haidilao():
    ...
    def process_request(self):
        self.browser.get("http://tcx.haidilao.net:9084/TzxMember/")
        WebDriverWait(self.browser,10,0.5).until(EC.element_to_be_clickable((By.ID,'d3')))
        username,passwd = self.user_info()
        #尝试登陆
        try:
            self.browser.find_element_by_id("d3").send_keys(username)
            sleep(2)
            self.browser.find_element_by_xpath("//input[@id='d1']").send_keys(passwd)
            self.browser.find_element_by_xpath("//input[@id='d1']").send_keys(passwd)
            sleep(0.5)
    ...
    def get_cookies(self):
        ...
        with open(r"E:\code\spiders_three\cookie_bag\haidilao_cook.txt",'w') as fp:
    ...
    def account_info(self):
        ...
        with open(r"E:\code\spiders_three\cookie_bag\haidilao_cook.txt", 'r', encoding='utf-8') as fp:
